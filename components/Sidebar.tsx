...
import { Role, APP_LOGO_URL, RolePermission, STAFF_PERFORMANCE_PERMISSION_KEY } from '../types';
...
const MENU_GROUPS = [
  ...
  },
  {
    ...
    items: [
      ...
      { label: 'Duyệt mổ', icon: CheckCircle, path: '/surgery-approval', roles: [Role.CHIEF, Role.HEAD_NURSE, Role.STAFF] },
  ...
  },
  {
    ...
    items: [
      ...
      { label: 'Công việc & Đánh giá', icon: ClipboardCheck, path: '/staff-performance', roles: [], permissionKey: STAFF_PERFORMANCE_PERMISSION_KEY },
  ...
  },
  {
    title: 'V. Điều dưỡng',
    items: [
      { label: 'Công tác điều dưỡng', icon: HeartPulse, path: '/nursing-tasks', roles: [Role.HEAD_NURSE, Role.STAFF] },
...
export const Sidebar: React.FC<SidebarProps> = ({ isOpen, onClose, userRole }) => {
  ...
  useEffect(() => {
    ...
    getPermissions()
      .then((data) => {
        if (mounted) setPermissions(Array.isArray(data) ? data : []);
      })
      .catch(() => {
  ...
  const hasPermission = (allowedRoles: Role[], permissionKey?: string) => {
    if (allowedRoles.length === 0) return true;
    return allowedRoles.includes(userRole);
  };

  const hasModuleView = (permissionKey?: string) => {
    if (!permissionKey) return true;
    if (userRole === Role.CHIEF) return true;
    return permissions.some(
      (permission) =>
        permission.role === userRole &&
        permission.module === permissionKey &&
        toBool(permission.canView),
    );
  };
      ...
      >
        ...
        <div className="flex flex-col p-4 overflow-y-auto h-[calc(100vh-4rem)]">
          {MENU_GROUPS.map((group, groupIdx) => {
            const visibleItems = group.items.filter((item) => hasPermission(item.roles) && hasModuleView(item.permissionKey));
